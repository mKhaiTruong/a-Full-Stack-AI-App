...
export const uploadImageToHosting = async ({ hosting, url, projectId, label }: StoreHostedImageParams): Promise<HostedAsset | null> => {
  ...
  try {
    const resolved =
      label === "rendered" ? await imageUrlToPngBlob(url).then((blob) => (blob ? { blob, contentType: "image/png" } : null)) : await fetchBlobFromUrl(url);
    ...
    console.warn("Failed to store the hosting image:", error);
    return null;
  }
};
